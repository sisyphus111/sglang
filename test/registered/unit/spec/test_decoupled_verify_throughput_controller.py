...
class TestProfileCtxLensParser(unittest.TestCase):
...
class TestDecoupledVerifyScoring(unittest.TestCase):
    ...
    def test_scoring_passes_runtime_ctx_len_to_lookup(self):
        ...
        self.assertEqual(rows[0]["cost_ms"], 20.0)
...
class TestDecoupledVerifyThroughputAwareController(unittest.TestCase):
    ...
    def test_shrink_preserves_higher_position_windows(self):
        ...
        controller.set_profile_cost(batch_size=4, steps=3, ctx_len=256, cost_ms=100.0)
        controller.init_states(cuda_graph_bs=[4])
