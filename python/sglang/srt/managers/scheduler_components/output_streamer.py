...
class _GenerationStreamAccumulator:
    ...
    spec_valid_draft_tokens: Optional[list] = None
    spec_valid_accepted_tokens: Optional[list] = None
    spec_valid_draft_tokens_by_position: Optional[list] = None
    spec_valid_accepted_tokens_by_position: Optional[list] = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.spec_algorithm.is_decoupled_verify():
            self.spec_valid_draft_tokens = []
            self.spec_valid_accepted_tokens = []
            self.spec_valid_draft_tokens_by_position = []
            self.spec_valid_accepted_tokens_by_position = []
    ...
    def accept(self, *, req: Req) -> None:
        ...
        if (
            ...
            if self.spec_algorithm.is_decoupled_verify():
                assert self.spec_valid_draft_tokens is not None
                ...
                self.spec_valid_draft_tokens_by_position.append(
                    (list(req.spec_valid_draft_tokens_by_position) + [0] * spec_steps)[
                        :spec_steps
                    ]
                )
                self.spec_valid_accepted_tokens_by_position.append(
                    (
    ...
    def to_payload(
        ...
        return BatchTokenIDOutput(
            ...
            spec_valid_draft_tokens=self.spec_valid_draft_tokens,
            spec_valid_accepted_tokens=self.spec_valid_accepted_tokens,
