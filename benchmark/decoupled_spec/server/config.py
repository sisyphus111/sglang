...
def validate_role_pair(verifier: dict[str, Any], drafter: dict[str, Any]) -> None:
    ...
    if (
        not drafter_args.get("disable_overlap_schedule", False)
        and not drafter_args.get("disable_radix_cache", False)
    ):
        raise ValueError(
    ...
    # Both transports use the same GPU backend and wire format.
    cpp_data_plane_enabled(verifier)
    cpp_data_plane_enabled(drafter)
