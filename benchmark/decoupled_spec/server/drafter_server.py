"""Launch only the drafter HTTP server for an agent-driven run."""
...
def main() -> None:
    ...
    resolved = resolve_role_config(cli.config, "drafter", role_cli_overrides(cli))
    ...
    role_dir = run_dir / "server" / "drafter"
    role_dir.mkdir(parents=True, exist_ok=True)
    write_json(role_dir / "resolved_config.json", resolved)
    update_status(
        run_dir, "server/drafter", "starting", config=str(Path(cli.config).resolve())
    )
    try:
        ...
        update_status(run_dir, "server/drafter", "failed", error=repr(exc))
        raise
    finally:
        update_status(run_dir, "server/drafter", "exited")
