...
For dynamic logs, `model_source=scheduler_modeled_cost` means modeled ITL comes
from the selected runtime state printed by scheduler INFO and already contains
the runtime CPU overhead. Otherwise,
`model_source=profile_lookup_plus_overhead` means the analyzer performed
controller-aligned profile lookup and added the configured overhead.

`modeled_throughput_tok_s` has one meaning: the scheduler's `EMA modeled
throughput`, computed as `batch_size * EMA expected tokens * 1000 /
modeled_itl_ms`, where EMA expected tokens combines the current draft-supply and
conditional-accept position EMAs. The analyzer consumes this value directly and
never substitutes observed `accept_len`. Static logs and dynamic warm-up points
may leave it empty.

`trajectory_<case>.svg` (with a report-scale PNG companion) is the direct runtime view: raw and centered-smoothed
throughput, the controller-modeled throughput when available, raw and
